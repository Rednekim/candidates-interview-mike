...
public class CustomersDbSeeder
{
    ...
    private async Task InsertCustomersSampleData(CustomersDbContext db)
    {
        ...
        {
            _logger.LogError(@"Error in {CustomersSeeder}: {ExpMessage}", nameof(CustomersDbSeeder), exp.Message);
            throw;
        }
        ...
        {
            int numAffected = await db.SaveChangesAsync();
            _logger.LogInformation(@"Saved {NumAffected} customers", numAffected);
        }
        catch (Exception exp)
        {
            _logger.LogError(@"Error in {CustomersSeeder}: {ExpMessage}", nameof(CustomersDbSeeder), exp.Message);
    ...
    private List<Customer> GetCustomers(List<State> states)
    {
        ...
        var orders = new List<Order>
          {
              new() { Product = "Basket", Price = 29.99M, Quantity = 1 },
              new() { Product = "Yarn", Price = 9.99M, Quantity = 1 },
              new() { Product = "Needes", Price = 5.99M, Quantity = 1 },
              new() { Product = "Speakers", Price = 499.99M, Quantity = 1 },
              new() { Product = "iPod", Price = 399.99M, Quantity = 1 },
              new() { Product = "Table", Price = 329.99M, Quantity = 1 },
              new() { Product = "Chair", Price = 129.99M, Quantity = 4 },
              new() { Product = "Lamp", Price = 89.99M, Quantity = 5 },
              new() { Product = "Call of Duty", Price = 59.99M, Quantity = 1 },
              new() { Product = "Controller", Price = 49.99M, Quantity = 1 },
              new() { Product = "Gears of War", Price = 49.99M, Quantity = 1 },
              new() { Product = "Lego City", Price = 49.99M, Quantity = 1 },
              new() { Product = "Baseball", Price = 9.99M, Quantity = 5 },
              new() { Product = "Bat", Price = 19.99M, Quantity = 1 }
          };
        ...
        for (var i = 0; i < customerNames.Length; i++)
        {
            ...
            {
                (firstOrder, lastOrder) = (lastOrder, firstOrder);
            }
